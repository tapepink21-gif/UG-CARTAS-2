...
async def generate_np_card(
    *,
    ...
    if animated:
        ...
        arr_final = np.array(final.convert("RGBA"), dtype=np.int16)
        arr_bg    = np.array(static_bg_for_diff.convert("RGBA"), dtype=np.int16)

        # Diferencia absoluta por canal RGB
        diff = np.abs(arr_final[:, :, :3] - arr_bg[:, :, :3]).max(axis=2)

        # Píxeles que son casi idénticos al fondo → alpha 0 (transparente)
        # Píxeles que difieren mucho del fondo → mantener alpha original
        alpha_new = np.where(diff < 12, 0, arr_final[:, :, 3]).astype(np.uint8)

        result = arr_final.copy().astype(np.uint8)
        result[:, :, 3] = alpha_new
        canvas_text_only = Image.fromarray(result, "RGBA")
