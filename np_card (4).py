...
async def generate_np_card(
    *,
    ...
    animated = art_is_animated or bg_animated or (art_is_animated and style.bg_type == "blur_art")
    ...
    if style.bg_type == "blur_art" and album_art:
        # Escalar manteniendo proporción y recortar al centro
        art_bg = _fill_crop(album_art.convert("RGB"), W, H)
        ...
        # Para GIF animado se aplicará frame a frame en el export; aquí solo el primer frame
        _bg_first = bg_frames[0][0] if bg_frames and bg_frames[0][0] else bg_img
        bg_resized = _fill_crop(_bg_first.convert("RGB"), W, H)
    ...
    draw.text((tx, title_y), song_display, font=font_title, fill=(*hex_to_rgb(style.text_primary), 255))
    ...
    draw.text((bx, body_y), artist_line, font=font_body, fill=(*hex_to_rgb(style.text_secondary), 200))
    ...
    if animated:
        ...
        for i, ((art_frame_img, art_dur), (bg_frame_img, bg_dur)) in enumerate(zip(art_frames, bg_frames)):
            ...
            # Reconstruir el fondo animado para este frame
            if style.bg_type == "blur_art" and art_frame_img is not None:
                # Fondo blur del cover animado — cada frame usa su propio frame del art
                bg_f = _fill_crop(art_frame_img.convert("RGB"), W, H)
                bg_f = bg_f.filter(ImageFilter.GaussianBlur(radius=style.bg_blur))
                bg_f = ImageEnhance.Brightness(bg_f).enhance(style.bg_brightness)
                base = Image.new("RGBA", (W, H), (0, 0, 0, 255))
                base.paste(bg_f.convert("RGBA"), (0, 0))
                # Pegar encima todo lo que no es fondo (overlay, texto, etc.) desde final
                frame_canvas = Image.alpha_composite(base, final)
            elif bg_animated and bg_frame_img:
                # Fondo GIF personalizado animado
                frame_canvas = final.copy()
                bg_f = _fill_crop(bg_frame_img.convert("RGB"), W, H)
                bg_f = ImageEnhance.Brightness(bg_f).enhance(style.bg_brightness)
                base = Image.new("RGBA", (W, H), (0,0,0,255))
                base.paste(bg_f.convert("RGBA"), (0, 0))
                frame_canvas = Image.alpha_composite(base, frame_canvas)
            else:
                frame_canvas = final.copy()
        ...
        output = io.BytesIO()
        # Quantize each frame with dithering for better GIF color quality
        def _quantize(img):
            return img.convert("RGB").quantize(colors=256, method=Image.Quantize.MEDIANCUT, dither=1)

        q_frames = [_quantize(f) for f in rendered_frames]
        q_frames[0].save(
            output,
            format="GIF",
            save_all=True,
        ...
        output = io.BytesIO()
        final.convert("RGBA").save(output, format="PNG", compress_level=1)
        output.seek(0)
        return output
