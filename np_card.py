...
async def generate_np_card(
    *,
    ...
    if style.bg_type == "blur_art" and album_art:
        ...
        bg_resized = _bg_first.convert("RGB").resize((W, H), Image.LANCZOS)
    ...
    if animated:
        ...
        for i, ((art_frame_img, art_dur), (bg_frame_img, bg_dur)) in enumerate(zip(art_frames, bg_frames)):
            ...
            if style.bg_type == "blur_art" and art_frame_img is not None:
                ...
                bg_f = art_frame_img.convert("RGB").resize((W, H), Image.LANCZOS)
                ...
                bg_f = bg_frame_img.convert("RGB").resize((W, H), Image.LANCZOS)
                bg_f = ImageEnhance.Brightness(bg_f).enhance(style.bg_brightness)
                base = Image.new("RGBA", (W, H), (0,0,0,255))
                base.paste(bg_f.convert("RGBA"), (0, 0))
            ...
            art_frame_rgba = art_frame_img.convert("RGBA").resize((style.art_size, style.art_size), Image.LANCZOS)
